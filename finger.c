...
void Finger_Close(uint8_t finger_m){												
	if((finger_m > 0) && (finger_m <= 6)){												
		...
		fingers_addr[2*(finger_m-1)+1]->PCOR  |=  fingers_mask[2*(finger_m-1)+1];	//enviar a motor 1 0
	}
}
...
void Finger_Open(uint8_t finger_m){
  if((finger_m > 0) && (finger_m <= 6)){	
		...
		fingers_addr[2*(finger_m-1)+1]->PSOR  |=  fingers_mask[2*(finger_m-1)+1];	//enviar a motor 0 1
	}
}
...
void Finger_Stop(uint8_t finger_m){
  if((finger_m > 0) && (finger_m <= 6)){	
		fingers_addr[2*(finger_m-1)]->PCOR  |=  fingers_mask[2*(finger_m-1)];
		fingers_addr[2*(finger_m-1)+1]->PCOR  |=  fingers_mask[2*(finger_m-1)+1];	//enviar a motor 0 0
	}
}
